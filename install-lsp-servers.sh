...
echo "${YELLOW}Installing os independent language servers...${RESET}"

echo "${YELLOW}Installing with npm${RESET}"
...
echo "${YELLOW}Installing with yarn${RESET}"
yarn global add yaml-language-server # yaml

echo "${YELLOW}Installing os dependent language servers${RESET}"
if [[ "$OSTYPE" == "darwin"* ]]; then
    # Mac OSX
    echo "${YELLOW}Installing with brew${RESET}"
    brew install terraform-ls # terraform
else
    # Expecting Ubuntu.
    echo "${YELLOW}Installing with apt${RESET}"
